    ...
    <>
      ...
      {!order && gaveUp && (
        <div className="sheet-sub" style={{ textAlign: 'center' }}>
          Payment went through - we're still finalizing your order. Check Orders in a moment if it doesn't show up here.
        </div>
      )}
      {order && (
        <>
          <div className="order-confirm-id">Order #{order.id}</div>
          <div className="sheet-sub" style={{ textAlign: 'center' }}>You'll receive your loyalty stamps when you pick up this order. Quote your name and order at pickup (details under Orders page)</div>
          ...
          {/* No "start preparing" prompt here — a preorder is typically
              placed 1-2 days ahead of collection, so asking staff to start
              right at payment makes no sense yet. That request lives in My
              Rewards instead, where the customer can trigger it whenever
