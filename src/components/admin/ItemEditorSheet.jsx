...
function RowsEditor({ label, rows, setRows, addLabel, emptyNote }) {
...
export default function ItemEditorSheet({ cat, item, onClose, onSaved }) {
  ...
  return (
    <>
      ...
      <div className="field"><label htmlFor="item-name">Name</label><input id="item-name" value={name} onChange={e => setName(e.target.value)} /></div>
      <div className="field"><label htmlFor="item-desc">Description</label><textarea id="item-desc" rows={2} value={desc} onChange={e => setDesc(e.target.value)} /></div>
      <div className="field"><label htmlFor="item-price">Price ($)</label><input id="item-price" type="number" step="0.10" value={price} onChange={e => setPrice(e.target.value)} /></div>
      <div className="field"><label htmlFor="item-category">Category</label><input id="item-category" value={category} onChange={e => setCategory(e.target.value)} /></div>
      <div className="field">
        <label htmlFor="item-photo">Thumbnail photo</label>
        ...
        <input id="item-photo" type="file" accept="image/jpeg,image/png" onChange={handlePhotoFile} disabled={uploading} />
      ...
      <div className="field">
        <label htmlFor="item-preorder-limit">Preorder stock limit this week{item?.preorderSold ? ` (${item.preorderSold} sold)` : ''}</label>
        <input id="item-preorder-limit" type="number" min="0" value={preorderLimit} placeholder="Leave blank for unlimited" onChange={e => setPreorderLimit(e.target.value)} />
      </div>
      <div className="field">
        <label htmlFor="item-walkin-limit">Walk-in stock limit this week{item?.walkinSold ? ` (${item.walkinSold} sold)` : ''}</label>
        <input id="item-walkin-limit" type="number" min="0" value={walkinLimit} placeholder="Leave blank for unlimited" onChange={e => setWalkinLimit(e.target.value)} />
      </div>
